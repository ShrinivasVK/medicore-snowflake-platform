-- ============================================================
-- MEDICORE HEALTH SYSTEMS — PHASE 09: AUDIT & COMPLIANCE
-- ============================================================
-- Script:      09_audit.sql
-- Version:     1.0.0
...
--   - Compliance monitoring (42 CFR Part 2, HIPAA)
...
-- HIPAA COMPLIANCE:
-- -----------------
-- These audit views support HIPAA requirements for:
--   - Access logging (§164.312(b))
--   - Audit controls (§164.312(b))
--   - Authentication tracking (§164.312(d))
--
-- Prerequisites:
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_LOGIN_HISTORY
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_ROLE_GRANT_HISTORY
...
-- Retention: Last 180 days
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_PRIVILEGE_ESCALATION_EVENTS
...
-- ============================================================
-- Track access to sensitive data, particularly PHI and
-- data protected under 42 CFR Part 2.
-- ============================================================
...
-- access to DIRECT_IDENTIFIER and 42CFR_PART2 data.
--
...
-- Retention: Last 90 days
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_PHI_DATA_ACCESS
    COMMENT = 'PHI data access audit tracking queries to columns tagged as DIRECT_IDENTIFIER or 42CFR_PART2. Critical for HIPAA compliance. Data latency: up to 3 hours. Retention: 90 days.'
AS
SELECT
    ...
    CASE
        WHEN tr.TAG_VALUE = '42CFR_PART2' THEN 'CRITICAL'
        WHEN tr.TAG_VALUE = 'DIRECT_IDENTIFIER' THEN 'HIGH'
        WHEN tr.TAG_VALUE = 'QUASI_IDENTIFIER' THEN 'MEDIUM'
        ELSE 'LOW'
    END                                                     AS SENSITIVITY_LEVEL,
    CASE
        WHEN qh.ROLE_NAME NOT IN ('MEDICORE_COMPLIANCE_OFFICER', 'ACCOUNTADMIN')
             AND tr.TAG_VALUE = '42CFR_PART2'
...
LEFT JOIN SNOWFLAKE.ACCOUNT_USAGE.TAG_REFERENCES tr
    ...
    AND tr.TAG_DATABASE = 'MEDICORE_GOVERNANCE_DB'
    AND tr.TAG_SCHEMA = 'TAGS'
    AND tr.TAG_NAME IN ('PHI_CLASSIFICATION', 'REGULATORY_FRAMEWORK')
    AND tr.TAG_VALUE IN ('DIRECT_IDENTIFIER', 'QUASI_IDENTIFIER', '42CFR_PART2')
WHERE qh.START_TIME >= DATEADD(DAY, -90, CURRENT_TIMESTAMP())
  AND qh.WAREHOUSE_NAME LIKE 'MEDICORE_%'
...
-- Data Latency: Up to 3 hours
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_MASKING_POLICY_USAGE
...
SELECT
    ...
    CASE
        WHEN pr.POLICY_NAME LIKE '%42CFR%' THEN 'CRITICAL'
        WHEN pr.POLICY_NAME LIKE '%DIRECT%' THEN 'HIGH'
        WHEN pr.POLICY_NAME LIKE '%SENSITIVE%' THEN 'HIGH'
        ELSE 'STANDARD'
...
WHERE pr.POLICY_KIND = 'MASKING_POLICY'
  AND pr.REF_DATABASE_NAME LIKE 'MEDICORE_%'
...
-- Data Latency: Up to 3 hours
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_ROW_ACCESS_POLICY_USAGE
...
SELECT
    ...
    CASE
        WHEN pr.POLICY_NAME LIKE '%CLINICAL%' THEN 'HIGH'
        WHEN pr.POLICY_NAME LIKE '%CONSENT%' THEN 'CRITICAL'
        WHEN pr.POLICY_NAME LIKE '%ENVIRONMENT%' THEN 'MEDIUM'
        ELSE 'STANDARD'
    END                                                     AS PROTECTION_LEVEL
FROM SNOWFLAKE.ACCOUNT_USAGE.POLICY_REFERENCES pr
...
-- Retention: Last 180 days
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_TAG_CHANGE_HISTORY
...
-- Retention: Last 180 days
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_POLICY_CHANGE_HISTORY
...
-- Retention: Last 90 days
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_QUARANTINED_DATA_ACCESS_ATTEMPTS
...
SELECT
    ...
    'HIGH'                                                  AS SEVERITY,
    CASE
        WHEN qh.ROLE_NAME IN ('MEDICORE_DATA_ENGINEER', 'MEDICORE_PLATFORM_ADMIN', 'ACCOUNTADMIN')
...
-- ------------------------------------------------------------
-- VIEW: V_42CFR_ACCESS_ATTEMPTS
-- ------------------------------------------------------------
-- Monitors all access attempts to data protected under
...
--
-- Flags access by roles other than COMPLIANCE_OFFICER.
--
-- Data Latency: Up to 3 hours
-- Retention: Last 90 days
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_42CFR_ACCESS_ATTEMPTS
    COMMENT = '42 CFR Part 2 compliance monitoring tracking all access to substance abuse treatment data. Flags non-compliance officer access. Data latency: up to 3 hours. Retention: 90 days.'
AS
SELECT
    qh.QUERY_ID                                             AS QUERY_ID,
    qh.START_TIME                                           AS ACCESS_TIMESTAMP,
    ...
    LEFT(qh.QUERY_TEXT, 500)                                AS QUERY_TEXT_PREVIEW,
    qh.EXECUTION_STATUS                                     AS EXECUTION_STATUS,
    '42CFR_PART2_ACCESS'                                    AS REGULATORY_CATEGORY,
    CASE
        WHEN qh.ROLE_NAME IN ('MEDICORE_COMPLIANCE_OFFICER', 'ACCOUNTADMIN')
        THEN 'AUTHORIZED'
    ...
    CASE
        WHEN qh.ROLE_NAME NOT IN ('MEDICORE_COMPLIANCE_OFFICER', 'ACCOUNTADMIN')
        THEN 'Verify patient consent exists for this access'
        ELSE 'Authorized compliance access'
...
WHERE qh.START_TIME >= DATEADD(DAY, -90, CURRENT_TIMESTAMP())
  ...
  AND tr.TAG_DATABASE = 'MEDICORE_GOVERNANCE_DB'
  AND tr.TAG_SCHEMA = 'TAGS'
  AND tr.TAG_NAME = 'REGULATORY_FRAMEWORK'
  AND tr.TAG_VALUE = '42CFR_PART2'
  AND qh.QUERY_TYPE IN ('SELECT', 'INSERT', 'UPDATE', 'MERGE', 'DELETE')
ORDER BY qh.START_TIME DESC;
...
-- ============================================================
-- SECTION 6: WAREHOUSE & SESSION AUDIT
-- ============================================================
...
--   - SYSADMIN
--
-- Critical for separation of duties compliance.
--
-- Data Latency: Up to 45 minutes
-- Retention: Last 90 days
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_ADMIN_ACTIVITY
    COMMENT = 'Administrative activity audit tracking all operations performed as ACCOUNTADMIN, SECURITYADMIN, or SYSADMIN. Data latency: up to 45 minutes. Retention: 90 days.'
AS
SELECT
    QUERY_ID                                                AS QUERY_ID,
...
--   +25  Access to DIRECT_IDENTIFIER data
--   +40  Access to 42CFR_PART2 data
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW MEDICORE_GOVERNANCE_DB.AUDIT.V_SECURITY_RISK_SCORE
    COMMENT = 'User security risk scoring based on admin usage, privilege escalations, PHI access, and failed logins. Risk levels: LOW (0-25), MEDIUM (26-50), HIGH (51-100), CRITICAL (101+).'
AS
WITH admin_usage AS (
    SELECT 
        USER_NAME,
...
direct_identifier_access AS (
    ...
    WHERE qh.START_TIME >= DATEADD(DAY, -90, CURRENT_TIMESTAMP())
      ...
      AND tr.TAG_VALUE = 'DIRECT_IDENTIFIER'
    GROUP BY qh.USER_NAME
),
cfr42_access AS (
    SELECT
        qh.USER_NAME,
        COUNT(*) AS cfr42_access_count,
        40 AS risk_points
    FROM SNOWFLAKE.ACCOUNT_USAGE.QUERY_HISTORY qh
    JOIN SNOWFLAKE.ACCOUNT_USAGE.TAG_REFERENCES tr
        ON tr.OBJECT_DATABASE = qh.DATABASE_NAME
    WHERE qh.START_TIME >= DATEADD(DAY, -90, CURRENT_TIMESTAMP())
      AND tr.TAG_NAME = 'REGULATORY_FRAMEWORK'
      AND tr.TAG_VALUE = '42CFR_PART2'
      AND qh.ROLE_NAME NOT IN ('MEDICORE_COMPLIANCE_OFFICER', 'ACCOUNTADMIN')
    GROUP BY qh.USER_NAME
),
failed_logins AS (
    SELECT
...
SELECT
    ...
    COALESCE(dia.phi_access_count, 0)                       AS PHI_ACCESS_COUNT,
    COALESCE(ca.cfr42_access_count, 0)                      AS CFR42_ACCESS_COUNT,
    ...
    (
        ...
        CASE WHEN dia.USER_NAME IS NOT NULL THEN dia.risk_points ELSE 0 END +
...
LEFT JOIN direct_identifier_access dia ON u.USER_NAME = dia.USER_NAME
LEFT JOIN cfr42_access ca ON u.USER_NAME = ca.USER_NAME
...
-- administration. Restrict from clinical and analyst roles.
-- ============================================================

-- Grant SELECT on all audit views to COMPLIANCE_OFFICER
GRANT SELECT ON ALL VIEWS IN SCHEMA MEDICORE_GOVERNANCE_DB.AUDIT
    TO ROLE MEDICORE_COMPLIANCE_OFFICER;

-- Grant SELECT on all audit views to PLATFORM_ADMIN
GRANT SELECT ON ALL VIEWS IN SCHEMA MEDICORE_GOVERNANCE_DB.AUDIT
    TO ROLE MEDICORE_PLATFORM_ADMIN;

-- Grant future view access
GRANT SELECT ON FUTURE VIEWS IN SCHEMA MEDICORE_GOVERNANCE_DB.AUDIT
    TO ROLE MEDICORE_COMPLIANCE_OFFICER;
...
-- List all views in AUDIT schema
SHOW VIEWS IN SCHEMA MEDICORE_GOVERNANCE_DB.AUDIT;

-- Verify grants to COMPLIANCE_OFFICER
