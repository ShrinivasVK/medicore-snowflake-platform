...
WITH inpatient_encounters AS (
    SELECT
        ...
        LENGTH_OF_STAY_DAYS, ENCOUNTER_MONTH, DISCHARGE_MONTH
    FROM MEDICORE_ANALYTICS_DB.DEV_CLINICAL.ENCOUNTERS
    WHERE IS_INPATIENT_FLAG = 1
      AND ADMISSION_DATE IS NOT NULL
      AND DISCHARGE_DATE IS NOT NULL
),
readmission_logic AS (
    SELECT *, 
        CASE WHEN LEAD(ADMISSION_DATE) OVER (PARTITION BY PATIENT_ID ORDER BY ADMISSION_DATE)
             BETWEEN DATEADD('DAY', 1, DISCHARGE_DATE) AND DATEADD('DAY', 30, DISCHARGE_DATE)
        THEN 1 ELSE 0 END AS IS_READMISSION_CASE
...
monthly_inpatient_aggregation AS (
    SELECT
        ...
        ROUND(SUM(IS_READMISSION_CASE)::FLOAT / NULLIF(COUNT(ENCOUNTER_ID), 0) * 100, 2) AS READMISSION_RATE_PERCENT
...
lab_aggregation AS (
    SELECT
        ...
        ROUND(SUM(CASE WHEN IS_ABNORMAL_FLAG = TRUE THEN 1 ELSE 0 END)::FLOAT / NULLIF(COUNT(LAB_RESULT_ID), 0) * 100, 2) AS ABNORMAL_LAB_RATE_PERCENT
...
FULL OUTER JOIN lab_aggregation la ON mia.MONTH_KEY = la.MONTH_KEY
ORDER BY MONTH_KEY;
...
--         DISCHARGE_MONTH
--     FROM MEDICORE_ANALYTICS_DB.DEV_CLINICAL.ENCOUNTERS
--     WHERE IS_INPATIENT_FLAG = 1  -- Fixed: use 1 instead of TRUE
--       AND ADMISSION_DATE IS NOT NULL
--       AND DISCHARGE_DATE IS NOT NULL
-- ),
-- readmission_logic AS (
...
--             ) BETWEEN DATEADD('DAY', 1, DISCHARGE_DATE) 
--                   AND DATEADD('DAY', 30, DISCHARGE_DATE)
--             THEN 1 
--             ELSE 0 
